import { connectDB, db, isMockDatabase } from './models/db';
import { IntakeAgent, GrantAgent, GuardianAgent } from './services/AgentServices';
...
async function runTests() {
  ...
  const users = await IntakeAgent.setupFamily(
    familyId,
    { name: 'Jo', email: parentEmail, passwordHash: 'hash' },
    [
  ...
  const accounts = await db.accounts.find({ familyId });
  const checking = accounts.find(a => a.accountName === 'Checking Account')!;
  const bills = await db.bills.find({ familyId });
  ...
  const resDeniedNoGrant = await GuardianAgent.checkAndExecute(delegate._id, {
    type: 'view_balance',
    domain: 'financial',
  ...
  const grant = await GrantAgent.createGrant({
    parentId: parent._id,
  ...
  const resSmallBillPay = await GuardianAgent.checkAndExecute(delegate._id, {
    type: 'pay_bill',
    domain: 'household',
  ...
  if (resSmallBillPay.status === 'approved') {
    ...
    const updatedChecking = await db.accounts.findById(checking.id);
  ...
  const resLargeBillPay = await GuardianAgent.checkAndExecute(delegate._id, {
    type: 'pay_bill',
    domain: 'household',
  ...
  const checkInitial = await verifyAuditChain();
  ...
  if (isMockDatabase) {
    ...
    const mongooseModel = (db.audit as any).UserModel || (mongooseModel => {
  ...
  // Run validation
  const checkTampered = await verifyAuditChain();
